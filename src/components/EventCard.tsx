...
export default function EventCard({ event }: Props) {
  ...
  const toggleSave = (e: React.MouseEvent) => {
    ...
    if (!user) {
      toast({
        title: "Inicia sesión",
        description: "Debes estar conectado para guardar tus eventos favoritos.",
        variant: "destructive"
      });
      return;
    }
    ...
    saveEventMutation.mutate({ userId: user.id, eventId: event.id, isSaved }, {
      onSuccess: () => {
        toast({
          title: isSaved ? "Eliminado de favoritos" : "¡Añadido!",
          description: isSaved ? "Has quitado este evento de tus favoritos." : "Hemos guardado este evento en tu lista de favoritos.",
        });
      }
    });
  };
      ...
      >
        <div className="relative h-64 overflow-hidden">
          ...
          <div className="absolute top-4 right-4 flex gap-2">
            ...
            <button
              onClick={toggleSave}
              className={`p-2.5 rounded-xl backdrop-blur-md shadow-lg transition-all duration-300 ${isSaved
          ...
          <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                {attendanceCount > 0 ? `+${attendanceCount} lo tienen en favoritos` : 'Guardar favorito'}
              </span>
            </div>
            ...
            <div className="md:w-1/2 p-12 overflow-y-auto custom-scrollbar bg-white flex flex-col">
              ...
              <div className="mt-10 pt-10 border-t border-slate-100 flex flex-col gap-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-slate-500">
                      <span className="text-slate-900 mr-1">{attendanceCount + 15} personas</span> lo han guardado
                    </p>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  <button
                    onClick={toggleSave}
                    className={`flex-grow py-5 rounded-2xl font-black text-lg transition-all shadow-xl active:scale-95 flex items-center justify-center gap-3 ${isSaved ? 'bg-slate-100 text-slate-900 hover:bg-slate-200' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-blue-200'
                      }`}
                  >
                    {isSaved ? (
                      <>
